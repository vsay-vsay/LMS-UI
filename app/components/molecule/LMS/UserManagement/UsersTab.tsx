...
import { fetchAllUsers } from "~/routes/LMS/UserManagement/api";

const LMSUserManagementMolecule = () => {
    ...
    const columns = useMemo(() => [
        {
            ...
            accessor: "name",
            Cell: ({ row }) => (
                <Link to={`/lms/user-management/${row.original._id}`}>
                    {row.original.name}
                </Link>
            )
        },
                        ...
                        {activeDropdown === row.index && (
                            <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-50">
                                <div className="py-1">
                                    ...
                                    <button
                                        onClick={() => navigate(`/lms/user-management/${row.original._id}`)}
                                        className="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 w-full"
                                    >
                                        <FiEye className="mr-2" /> View Details
                                    </button>
                                    <AlertDelete email={row.original.email} id={row.original._id} onSuccess={() => setRefreshKey(prev => prev + 1)}>
                                        <button
        ...
        <div className="p-4 bg-white rounded-lg shadow-lg border ml-6">
            ...
            {filteredData.length === 0 ? (
                ...
                <Table
                    columns={columns}
                    data={filteredData}
                    onViewClick={(id) => navigate(`/lms/user-management/${id}`)}
                    onDeleteClick={(id) => {
...
export default LMSUserManagementMolecule;