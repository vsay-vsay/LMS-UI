...
const ERPClassManagementMolecule = () => {
    ...
    const filteredData = useMemo(() => {
        return data.filter(classItem =>
            ...
            classItem.createdBy.name.toLowerCase().includes(searchTerm.toLowerCase())
        );
    }, [data, searchTerm]);
                        ...
                        {activeDropdown === row.index && (
                            <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-50">
                                <div className="py-1">
                                    ...
                                    <button
                                        onClick={() => navigate(`/erp/class-management/${row.original._id}`)}
                                        className="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 w-full"
                                    >
                                        <FiEye className="mr-2" /> View Details
                                    </button>
    ...
    const EmptyState = () => (
        ...
        >
            ...
            <h3 className="text-lg font-medium text-gray-900 pt-5">No Classes Available</h3>
